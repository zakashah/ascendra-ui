...
const errorRateData = [
  ...
  { d: "25", deploys: 1, errorRate: 0.40 },
  { d: "26", deploys: 4, errorRate: 0.40 },
...
const slaConfig: ChartConfig = { SLA: { label: "Uptime", color: "var(--chart-2)" } };

// ─── Recent Incidents ─────────────────────────────────────────────────────────

const incidents = [
  { id: "INC-0412", service: "Data Pipeline",  severity: "High",   started: "May 13 02:14", duration: "2h 18m",  status: "Resolved",    responder: "jsmith" },
  { id: "INC-0411", service: "Notifications",   severity: "Medium", started: "May 12 18:41", duration: "54m",     status: "Resolved",    responder: "amurray" },
  { id: "INC-0410", service: "API Gateway",     severity: "Low",    started: "May 10 09:22", duration: "12m",     status: "Resolved",    responder: "lchen" },
  { id: "INC-0409", service: "Worker Queue",    severity: "Medium", started: "May 8 14:05",  duration: "38m",     status: "Resolved",    responder: "jsmith" },
  { id: "INC-0408", service: "Payments",        severity: "High",   started: "May 6 11:33",  duration: "1h 04m",  status: "Resolved",    responder: "dkowalski" },
  { id: "INC-0407", service: "Search",          severity: "Low",    started: "May 3 16:18",  duration: "8m",      status: "Resolved",    responder: "amurray" },
  { id: "INC-0413", service: "Data Pipeline",   severity: "Medium", started: "May 28 07:52", duration: "—",       status: "Investigating", responder: "lchen" },
];
    ...
    <div className="mx-auto max-w-7xl px-8 py-12">
      ...
      <div className="mb-10">
        ...
        <p className="max-w-2xl text-sm leading-relaxed text-muted-foreground">
          Service health, latency, error rates, and deploy velocity — the reliability view for a platform engineering team.
        </p>
      </div>
        ...
        <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
          {kpis.map((kpi) => (
            <Card key={kpi.label} className="h-full overflow-hidden border-l-[3px] border-l-orange-500/60">
              <CardPanel>
                <div className="flex flex-1 flex-col p-5 bg-linear-to-b from-orange-500/[0.07] to-transparent">
        ...
        <div className="grid grid-cols-12 gap-4">
          <div className="col-span-12">
            <Card className="h-full">
              ...
              <CardPanel>
                <div className="p-5">
                  <ChartContainer config={requestVolumeConfig} className="h-64 w-full">
                    <AreaChart
                    ...
                    >
                      <defs>
                        {(["api", "auth", "storage", "worker"] as const).map((key, i) => (
                      ...
                      />
                      <Area type="monotone" dataKey="api"     stackId="1" stroke="var(--chart-1)" fill={`url(#grad-api)`}     strokeWidth={1.5} hide={hiddenVolume["api"]} />
                      <Area type="monotone" dataKey="auth"    stackId="1" stroke="var(--chart-2)" fill={`url(#grad-auth)`}    strokeWidth={1.5} hide={hiddenVolume["auth"]} />
                      <Area type="monotone" dataKey="storage" stackId="1" stroke="var(--chart-3)" fill={`url(#grad-storage)`} strokeWidth={1.5} hide={hiddenVolume["storage"]} />
                      <Area type="monotone" dataKey="worker"  stackId="1" stroke="var(--chart-4)" fill={`url(#grad-worker)`}  strokeWidth={1.5} hide={hiddenVolume["worker"]} />
                    </AreaChart>
                  </ChartContainer>
                  <ChartSeriesLegend
                    config={requestVolumeConfig}
                    hidden={hiddenVolume}
                    onToggle={(k) => setHiddenVolume((p) => ({ ...p, [k]: !p[k] }))}
                    ...
                    <ChartContainer config={latencyConfig} className="h-52 w-full">
                      ...
                      >
                        ...
                        <ReferenceLine
                          ...
                          strokeDasharray="4 3"
                          strokeOpacity={0.6}
                          label={{ value: "SLA 200ms", position: "insideTopRight", fontSize: 10, fill: "var(--destructive)" }}
                        />
                        <ChartTooltip
                          content={
                            <ChartTooltipContent
                        ...
                        />
                        <Line type="monotone" dataKey="api"    stroke="var(--chart-1)" strokeWidth={2} dot={false} activeDot={{ r: 4 }} hide={hiddenLatency["api"]} />
                        <Line type="monotone" dataKey="db"     stroke="var(--chart-2)" strokeWidth={2} dot={false} activeDot={{ r: 4 }} hide={hiddenLatency["db"]} />
                        <Line type="monotone" dataKey="cache"  stroke="var(--chart-3)" strokeWidth={2} dot={false} activeDot={{ r: 4 }} hide={hiddenLatency["cache"]} />
                        <Line type="monotone" dataKey="worker" stroke="var(--chart-4)" strokeWidth={2} dot={false} activeDot={{ r: 4 }} hide={hiddenLatency["worker"]} />
                      </LineChart>
                    </ChartContainer>
                  </div>
                  <ChartSeriesLegend
                    ...
                    onToggle={(k) => setHiddenLatency((p) => ({ ...p, [k]: !p[k] }))}
                    className="mt-3 w-full"
                  />
                </div>
              </CardPanel>
            ...
            <Card className="h-full">
              <CardPanel>
                <div className="flex flex-1 flex-col p-5">
                  ...
                  <div className="flex-1 min-h-0">
                    <ChartContainer config={errorsConfig} className="h-52 w-full">
                      <BarChart
                      ...
                      >
                        ...
                        <Bar dataKey="errors" radius={[0, 3, 3, 0]}>
                          {errorsByServiceData.map((entry, idx) => (
                            <Cell
                              key={`cell-${idx}`}
                              fill={entry.errors > ERROR_THRESHOLD ? "var(--destructive)" : "var(--chart-1)"}
                              fillOpacity={0.82}
                            />
                          ))}
                        </Bar>
                  ...
                  <p className="mt-2 text-xs text-muted-foreground">
                    Threshold: {ERROR_THRESHOLD} errors/week · red bars exceed limit
                  </p>
                </div>
              </CardPanel>
            </Card>
        ...
        <div className="grid grid-cols-12 gap-4">
          ...
          <div className="col-span-12 md:col-span-8">
            <Card className="h-full">
              <CardPanel>
                <div className="p-5">
                  <p className="mb-4 text-sm font-medium">Error Rate & Deploy Frequency</p>
                  <ChartContainer config={errorRateConfig} className="h-52 w-full">
                    <ComposedChart
                      data={errorRateData}
                    ...
                    >
                      ...
                      <YAxis
                        ...
                        width={32}
                        label={{ value: "Deploys", angle: -90, position: "insideLeft", fontSize: 10, fill: "var(--muted-foreground)", dy: 30 }}
                      />
                      <YAxis
                        yAxisId="right"
                        orientation="right"
                      ...
                      <ChartTooltip
                        content={
                          <ChartTooltipContent
                            formatter={makeTooltipFormatter(
                              errorRateConfig,
                              (v, k) => k === "errorRate" ? `${(v as number).toFixed(2)}%` : v.toString(),
                            )}
                          />
                        }
                      />
                  ...
                  <ChartSeriesLegend
                    ...
                    onToggle={(k) => setHiddenErrRate((p) => ({ ...p, [k]: !p[k] }))}
                    className="mt-3 w-full"
                  />
                </div>
              </CardPanel>
                ...
                <div className="flex flex-1 flex-col items-center justify-center p-6">
                  <ChartContainer config={slaConfig} className="h-44 w-44">
                    <RadialBarChart
                      ...
                      barSize={14}
                    >
                      <PolarAngleAxis type="number" domain={[99.0, 100]} tick={false} />
                      <RadialBar
                        dataKey="value"
                        cornerRadius={7}
                ...
                <TableBody>
                  {incidents.map((row) => (
                    <TableRow key={row.id}>
                      ...
                      <TableCell className="text-muted-foreground tabular-nums">{row.started}</TableCell>
                      <TableCell className="tabular-nums">{row.duration}</TableCell>
                      <TableCell>
                        <SimpleBadge
                          variant={row.status === "Resolved" ? "green" : "amber"}
                        >
                          {row.status}
                        </SimpleBadge>
                      </TableCell>
                      <TableCell className="font-mono text-xs text-muted-foreground">{row.responder}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
