...
const medications = [
  { drug: "Lisinopril",    dose: "10 mg",     freq: "Once daily (morning)",       prescriber: "Dr. Chen",  start: "15 Jan 2023" },
  { drug: "Atorvastatin",  dose: "20 mg",     freq: "Once daily (evening)",       prescriber: "Dr. Chen",  start: "15 Jan 2023" },
  { drug: "Metformin",     dose: "500 mg",    freq: "Twice daily (with meals)",   prescriber: "Dr. Patel", start: "02 Jun 2022" },
  { drug: "Vitamin D3",    dose: "2,000 IU",  freq: "Once daily",                 prescriber: "Self",      start: "10 Mar 2024" },
];

type LabStatus = "normal" | "borderline" | "abnormal";

const labs: { test: string; result: string; range: string; status: LabStatus }[] = [
  { test: "HbA1c",             result: "6.8 %",       range: "< 5.7% (normal), < 7.0% (controlled)", status: "normal"     },
  { test: "Fasting Glucose",   result: "112 mg/dL",   range: "70 – 99 mg/dL",                         status: "borderline" },
  { test: "Total Cholesterol", result: "172 mg/dL",   range: "< 200 mg/dL",                           status: "normal"     },
  { test: "LDL Cholesterol",   result: "98 mg/dL",    range: "< 100 mg/dL",                           status: "normal"     },
  { test: "HDL Cholesterol",   result: "52 mg/dL",    range: "> 40 mg/dL (male)",                     status: "normal"     },
  { test: "Triglycerides",     result: "148 mg/dL",   range: "< 150 mg/dL",                           status: "normal"     },
  { test: "eGFR",              result: "78 mL/min",   range: "> 60 mL/min",                           status: "normal"     },
];

// ─── Status styling ────────────────────────────────────────────────────────────

const statusCls: Record<LabStatus | "normal" | "borderline" | "abnormal", string> = {
  normal:     "bg-emerald-500/10 text-emerald-700 ring-emerald-500/20 dark:text-emerald-400 dark:ring-emerald-500/30",
  borderline: "bg-amber-500/10 text-amber-700 ring-amber-500/20 dark:text-amber-400 dark:ring-amber-500/30",
  abnormal:   "bg-rose-500/10 text-rose-700 ring-rose-500/20 dark:text-rose-400 dark:ring-rose-500/30",
};
      ...
      <div className="flex flex-col gap-10">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
            {vitals.map((v) => (
              ...
              >
                <span className="text-[0.6875rem] text-muted-foreground">{v.label}</span>
              ...
              <TableBody border={{}} bg={{}}>
                {medications.map((m) => (
                  <TableRow key={m.drug}>
                    ...
                    <TableCell className="tabular-nums text-muted-foreground">{m.dose}</TableCell>
                    <TableCell className="text-muted-foreground">{m.freq}</TableCell>
                    <TableCell className="text-muted-foreground">{m.prescriber}</TableCell>
                    <TableCell className="text-muted-foreground">{m.start}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
        ...
        <div>
          ...
          <div className="flex flex-col gap-5 text-sm leading-relaxed">
            <div>
              <p className="mb-1 font-semibold text-foreground">Reason for Visit</p>
              <p className="text-muted-foreground">
                Routine follow-up for management of Type 2 Diabetes Mellitus and hypertension.
                Patient attends every 3 months for monitoring and medication review.
              </p>
            </div>
            <div>
              <p className="mb-1 font-semibold text-foreground">Findings</p>
              <p className="text-muted-foreground">
                Patient reports good compliance with all prescribed medications. HbA1c at 6.8%
                remains within the controlled target range (&lt; 7.0%). Fasting glucose slightly
                elevated at 112 mg/dL; patient was counselled on dietary modifications, particularly
                reducing refined carbohydrate intake. Blood pressure borderline at 128/82 mmHg;
                patient counselled on sodium reduction and increased physical activity. All lipid
                panels within normal reference ranges, consistent with statin therapy response.
                No signs of renal deterioration (eGFR 78 mL/min). No new symptoms reported.
              </p>
            </div>
            <div>
              <p className="mb-1 font-semibold text-foreground">Assessment & Plan</p>
        ...
        <div className="flex flex-col gap-2 border-t pt-6 text-xs text-muted-foreground">
          <p>
            This document is a confidential patient health record generated by Ascendra Regional
            Medical Centre. It is intended solely for the treating physician and authorised clinical
            staff. Unauthorised disclosure is prohibited under applicable privacy law.
          </p>
          <div className="flex flex-wrap items-center justify-between gap-2 pt-1">
            <span className="font-medium text-foreground/60">Ascendra Regional Medical Centre</span>
